<template>
	<el-dialog :title="(ruleForm.dictCode !== 0 ? '修改' : '添加') + '字典'" v-model="isShowDialog" width="600px" :close-on-click-modal="false">
		<el-form :model="ruleForm" ref="formRef" :rules="rules" size="default" label-width="90px">
			...
			<el-form-item label="字典颜色" prop="dictColor">
				<el-color-picker v-model="ruleForm.dictColor" show-alpha/>
			</el-form-item>

			<el-form-item label="系统默认">
				<el-switch v-model="ruleForm.isDefault" inline-prompt active-text="是" inactive-text="否" :active-value="1" :inactive-value="0"></el-switch>
			</el-form-item>
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="onSubmit" class="dialog-footer-button">{{ ruleForm.dictCode !== 0 ? '修改' : '添加' }}</el-button>
			</span>
		</template>
	</el-dialog>
</template>
...
import { DictState, RuleFormDictState } from '/@/types/model';
...
export default defineComponent({
	...
	setup(prop, { emit }) {
		...
		// 打开弹窗
		const openDialog = (row: RuleFormDictState | null) => {
			resetForm();
			if (row) {
