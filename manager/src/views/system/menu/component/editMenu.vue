<template>
	<el-dialog :title="(acType === 'add' ? '新增' : '修改') + '菜单'" v-model="isShowDialog" width="750px" :close-on-click-modal="false">
					...
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb15">
						<el-form-item label="是否显示">
							<el-radio-group v-model="ruleForm.isHide">
								<el-radio v-for="dict in visibleOptions" :key="dict.value" :value="dict.value">{{ dict.label }}</el-radio>
							</el-radio-group>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb15">
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="onSubmit" class="dialog-footer-button" :loading="loading">{{ acType === 'add' ? '新增' : '修改' }}</el-button>
			</span>
		</template>
	</el-dialog>
</template>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const state = reactive({
			...
			// 表单校验
			rules: {
				parentId: [{ required: true, message: '父菜单不能为空' }],
				name: [{ required: true, message: '接口规则不能为空' }],
