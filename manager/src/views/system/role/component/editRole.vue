<template>
	<el-dialog :title="(formData.id === 0 ? '添加' : '修改') + '角色'" v-model="isShowDialog" width="600px" :close-on-click-modal="false">
		...
		<template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="onSubmit" class="dialog-footer-button" :loading="loading">{{ formData.id === 0 ? '新增' : '修改' }}</el-button>
			</span>
		</template>
	</el-dialog>
</template>
...
import { RoleDataRow, RoleState } from '/@/types/model';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		// 打开弹窗
		const openDialog = (row?: RoleDataRow) => {
			resetForm();
			getMenuData();
			if (row) {
