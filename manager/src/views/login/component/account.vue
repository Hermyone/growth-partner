<template>
	<el-form ref="loginForm" size="large" class="login-content-form" :model="ruleForm" :rules="formRules">
		...
		<el-form-item class="login-animation4">
			<el-button type="primary" class="login-content-submit" round @click="onSignIn" :loading="loading.signIn" v-waves="'light'">
...
export default defineComponent({
	...
	setup() {
		...
		const onSignIn = async () => {
			...
			formWrap.validate((valid: boolean) => {
              ...
              // console.log("登录成功", res)
							...
							userInfo.avatar = proxy.getUpFileUrl(userInfo.avatar ? userInfo.avatar: 'images/logo.png');
							// 存储 token 到浏览器缓存
							...
							// 记录密码，MD5
							Cookies.set('password', crypto.MD5(state.ruleForm.password));
		...
		const signInSuccess = () => {
			...
			// 登录成功，跳到转首页
			// 如果是复制粘贴的路径，非首页/登录页，那么登录成功后重定向到对应的路径中,如果redirect是401或404时跳转到首页
			if (route.query?.redirect && route.query?.redirect !== '/401' && route.query?.redirect !== '/404') {
				router.push({
