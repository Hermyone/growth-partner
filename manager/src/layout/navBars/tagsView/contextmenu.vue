...
export default defineComponent({
	...
	setup(props, { emit }) {
		const state = reactive({
			isShow: false,
			dropdownList: [
				{ contextMenuClickId: 0, txt: 'message.tagsView.refresh', affix: false, icon: 'ele-Refresh' },
				{ txt: '-', affix: true },
				...
				{ contextMenuClickId: 5, txt: 'message.tagsView.openNewWindow', affix: false, icon: 'ele-DocumentAdd' },
			],
			item: {},
			arrowLeft: 10,
		});
		...
		const openContextmenu = (item: any) => {
			state.item = item;
			item.meta.isAffix ? (state.dropdownList[2].affix = true) : (state.dropdownList[2].affix = false);
			closeContextmenu();
			setTimeout(() => {
				state.isShow = true;
			}, 10);
