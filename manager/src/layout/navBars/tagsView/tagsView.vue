<template>
  <div class="layout-navbars-tagsview" :class="{ 'layout-navbars-tagsview-shadow': getThemeConfig.layout === 'classic' }">
      <button class="tag-button ml5" :disabled="!enabledLeftArrow" @click="scrollTo(-300)">
          <el-icon><ele-ArrowLeft/></el-icon>
      </button>
      <ul class="layout-navbars-tagsview-ul" :class="getTagsStyle" ref="tagsUlRef">
        <li
            ...
            :ref="
                ...
                "
        >
          <i class="layout-navbars-tagsview-ul-li-iconfont" v-if="isActive(v)"></i>
      ...
      <button class="tag-button ml5" :disabled="!enabledRightArrow" @click="scrollTo(300)">
          <el-icon><ele-ArrowRight/></el-icon>
      </button>
      <button v-if="getTagsStyle == 'tags-style-three'" class="tag-button" @click="refreshCurrentView">
          <el-icon><ele-Refresh/></el-icon>
      </button>
<!--      <el-dropdown trigger="click">-->
<!--          <button class="tag-button mr5">-->
<!--              <el-icon><ele-Operation/></el-icon>-->
<!--          </button>-->
<!--          <template #dropdown>-->
<!--              <el-dropdown-menu style="max-height: 200px;">-->
<!--                  <el-dropdown-item v-for="(v, k) in tagsViewList"-->
<!--                                    :icon="v.meta.icon"-->
<!--                                    @click="onTagsClick(v, k)">-->
<!--                      {{ $t(v.meta.title) }}-->
<!--                  </el-dropdown-item>-->
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const tagsUlRef = ref();
      const enabledLeftArrow = ref(false);
      const enabledRightArrow = ref(false);
      ...
      // 滚动条滚动 step负数向左滚动  正数向右
      const scrollTo = (step: number) => {
        tagsUlRef.value.scrollTo({
          left: tagsUlRef.value.scrollLeft + step,
          behavior: 'smooth'
        })
      };
      // 滚动条初始化
      const scrollInit = () => {
        tagsUlRef.value.addEventListener("mousewheel", handler, false) ||
        tagsUlRef.value.addEventListener("DOMMouseScroll", handler, false);
        function handler(event:any) {
          const detail = event.wheelDelta || event.detail;
          const moveForwardStep = 1;
          const moveBackStep = -1;
          let step = 0;
          // 火狐上上滚键值-3 下滚键值3，其它内核上滚键值120 下滚键值-120
          if(detail == 3 || (detail < 0 && detail != -3)){
            step = moveForwardStep * 50;
          }else{
            step = moveBackStep * 50;
          }
          tagsUlRef.value.scrollLeft += step;
        }
        tagsUlRef.value.addEventListener("scroll", checkArrowShow, false);
        checkArrowShow();
      };
      const checkArrowShow = () => {
        if(tagsUlRef.value == null) return
        enabledLeftArrow.value = tagsUlRef.value.scrollLeft > 0;
        enabledRightArrow.value = tagsUlRef.value.clientWidth + tagsUlRef.value.scrollLeft < tagsUlRef.value.scrollWidth;
      };
      // 动态设置 tagsView 风格样式
      const getTagsStyle = computed(() => {
      ...
      const setTagsViewNameI18n = computed(() => {
        return (v: RouteItem) => {
          return common.setTagsViewNameI18n(v);
        };
      });
      // 设置 tagsView 高亮
      ...
      // 7、在新窗口打开
      const openNewWindow = async (path: string) => {
      ...
      const onCurrentContextmenuClick = async (item: RouteItem) => {
        ...
        switch (item.contextMenuClickId) {
          ...
          case 4:
            ...
            break;
          case 5:
            openNewWindow(getThemeConfig.value.isShareTagsView ? path : url);
            break;
        }
      };
      ...
      const tagsViewmoveToCurrentTag = () => {
        nextTick(() => {
          ...
          // 当前滚动条的值
          let scrollRefs = tagsUlRef.value;
          // 当前滚动条滚动宽度
      ...
      const onSortableResize = async () => {
        await initSortable();
        if (common.isMobile()) state.sortable.el && state.sortable.destroy();
      };
      // 页面加载前
      onBeforeMount(() => {
        // 初始化，防止手机端直接访问时还可以拖拽
        ...
        proxy.mittBus.on('onCurrentContextmenuClick', (data: RouteItem) => {
          data.isFnClick = true;
...
.layout-navbars-tagsview {
  ...
  // 分页下划线的颜色和粗细设置
  border-bottom: 1px solid var(--el-color-primary-light-8);// var(--next-border-color-light);
  position: relative;
  z-index: 4;
  display: flex;
  align-items: center;
