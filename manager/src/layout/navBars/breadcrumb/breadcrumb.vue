...
export default defineComponent({
	...
	setup() {
		...
		const initRouteSplit = (path: string) => {
			...
			if (state.breadcrumbList.length > 1) state.breadcrumbList.shift();
			if (state.breadcrumbList.length > 0) state.breadcrumbList[state.breadcrumbList.length - 1].meta.tagsViewName = common.setTagsViewNameI18n(route);
		};
		// 页面加载时
		onMounted(() => {
			initRouteSplit(route.path);
