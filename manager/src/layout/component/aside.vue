<template>
	<div class="h100" v-show="!isTagsViewCurrenFull">
		<el-aside class="layout-aside" :class="setCollapseStyle" v-if="(themeConfig.layout !== 'classic') || (themeConfig.layout === 'classic' && menuList && menuList.length > 1)">
			<Logo v-if="setShowLogo" />
			<div v-if="setShowTitle" >
				<h3 class="menu-title">{{ $t(menuTitle) }}</h3>
			</div>
...
export default defineComponent({
	...
	setup() {
		...
		const setCollapseStyle = computed(() => {
			...
			if (state.clientWidth <= 1000) {
				...
				if (layout === 'columns') {
					...
					else return [asideBrColor, 'layout-aside-pc-columns-220'];
				} else {
					// 其它布局给 64px
		...
		onBeforeMount(() => {
			...
			//改变菜单标题
			proxy.mittBus.on('setMenuTitle', (title: string) => {
