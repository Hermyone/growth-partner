...
<script setup lang="ts" name="navMenuHorizontal">
	import { defineAsyncComponent, reactive, computed, onMounted, nextTick, onBeforeMount, ref, getCurrentInstance } from 'vue';
	...
	const setSendClassicChildren = (path: string) => {
		const currentPathSplit : Array<string> = path.split('/');
		let currentData: MittMenu = { children: [] };
		filterRoutesFun(routesList.value).map((v, k) => {
			let idx = (currentPathSplit.length>2) ? currentPathSplit.length-2:1;
			let tmpPath = `/${currentPathSplit[idx]}`;
			// if(v.path.endsWith(tmpPath) || tmpPath.endsWith(v.path) || (v.path === `/${currentPathSplit[1]}`)) {
